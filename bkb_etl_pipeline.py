...
# print("PROCESS_CONTROL_TABLE : ",os.getenv("PROCESS_CONTROL_TABLE"))
# print("PROCESS_NAME : ",os.getenv("PROCESS_NAME"))
...
def load_incremental(config: dict, columns: list[dict]):
    ...
    # pk_col = next((c["column_name"] for c in columns if c["column_key"] == "PRI"), None)
    pk_col= primary_key
    if not pk_col:
        raise ValueError("No primary key found — cannot perform MERGE")

    non_pk_cols = [c["column_name"] for c in columns if c["column_name"] != pk_col]
        # non_pk_cols = [c["column_name"] for c in columns ]
        # non_pk_cols = [c["column_name"] for c in columns if c["column_key"] != "PRI"]

    update_set = ", ".join([f"{col} = EXCLUDED.{col}" for col in non_pk_cols])
        # update_set  = ", ".join([f"{c} = EXCLUDED.{c}" for c in non_pk_cols])

    # merge_sql = f"""
    #     INSERT INTO {tgt_schema}.{tgt_table}
    #     SELECT * FROM {tgt_schema}.{stg_table}
    #     ON CONFLICT ({pk_col})
...
def run_etl():
    ...
    try:
        ...
        # print(df)
        # print(tabulate(config, headers="keys", tablefmt="grid"))
